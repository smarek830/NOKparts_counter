...
# EMAIL (Nastav na True a vyplň údaje ak chceš používať)
...
class PocitadloApp:
    def __init__(self, root):
        ...
        # Dôležitá premenná: Ak je True, hlavná klávesnica nereaguje (lebo je otvorené popup okno)
        self.klavesnica_blokovana = False 
        
        # Štatistiky pre export
        self.stats = {} 
        ...
        # Vyžiadanie PINu po štarte (s malým oneskorením kvôli načítaniu UI)
        self.root.after(500, lambda: self.vyziadat_pin("PRIHLÁSENIE OBSLUHY", self.spracuj_prihlasenie))

    def nacitaj_obrazky(self):
        """Načíta PNG obrázky alebo vytvorí zástupné štvorce."""
        ikony = []
        for i in range(3):
            ...
            if os.path.exists(nazov_suboru):
                try:
                    ...
                    # Ak zlyhá načítanie, vytvorí štvorec
                    ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
            else:
                # Ak súbor neexistuje, vytvorí štvorec
                ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
        return ikony

    def vytvor_zastupny_obrazok(self, farba):
        # Veľkosť zástupného štvorca (ak chýba obrázok)
        img = tk.PhotoImage(width=300, height=300)
        img.put(farba, to=(0, 0, 299, 299))
        return img

    # --- POPUP OKNO (VÝBER CHYBY) ---
    def otvorit_vyber_chyby(self, index_riadku):
        ...
        # Kontajner pre stĺpce
        grid_container = tk.Frame(content, bg="white")
        # Tu je nastavená väčšia medzera od hlavičky (pady=(60, 5))
        ...
        # Generovanie 3 stĺpcov
        for i in range(3):
            col_frame = tk.Frame(grid_container, bg="white")
            col_frame.grid(row=0, column=i, sticky="nsew", padx=10) # Medzery medzi stĺpcami
            
            cfg = SUB_TYPES[i]
            
            # 1. TLAČIDLO S OBRÁZKOM (BEZ TEXTU)
        ...
        # Čakanie na viditeľnosť (prevencia Ghost Key)
    ...
    # --- LOGIKA UBERANIA (Jednorazový Undo) ---
    def odobrat_kus(self, index_riadku):
        ...
        # ZABLOKOVANIE PO JEDNOM POUŽITÍ
    ...
    # --- VYKRESLOVANIE BAROV (3 Segmenty) ---
    ...
    # --- POMOCNÉ FUNKCIE (Email, Varovanie, PIN, Menu) ---
    ...
    # --- OPRAVA SCHOVANÝCH DIALÓGOV (parent=parent_win) ---
