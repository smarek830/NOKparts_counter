...
# --- KONFIGURÁCIA CHÝB (Presne podľa zadania) ---
# Program hľadá obrázky s týmito názvami v rovnakej zložke
SUB_TYPES = [
...
class PocitadloApp:
    ...
    def nacitaj_obrazky(self):
        """Načíta obrázky definované v SUB_TYPES alebo vytvorí farebné štvorce."""
        ikony = []
        for i in range(3):
            ...
            if os.path.exists(nazov_suboru):
                try:
                    ...
                    ikony.append(img)
                    print(f"Obrázok načítaný: {nazov_suboru}")
                except Exception as e:
                    print(f"Chyba pri načítaní {nazov_suboru}: {e}")
                    ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
            else:
                print(f"Obrázok nenájdený: {nazov_suboru}, používam farbu.")
                ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
        return ikony
    ...
    # --- POPUP OKNO (Tadiaľto vedie jediná cesta k pridaniu kusu) ---
    def otvorit_vyber_chyby(self, index_riadku):
        print(f"DEBUG: Otváram okno pre riadok {index_riadku}")
        self.klavesnica_blokovana = True
        nazov_chyby = NAZVY_TLACIDIEL[index_riadku]
        
        popup = tk.Toplevel(self.root)
        popup.title("Detail chyby")
        ...
        def potvrdit_vyber(sub_id):
            print(f"DEBUG: Vybraný typ {sub_id}, zatváram okno a pridávam kus.")
            popup.destroy()
            self.klavesnica_blokovana = False
        ...
        # Generovanie 3 veľkých tlačidiel
        for i in range(3):
    ...
    # --- OVLÁDANIE KLÁVESNICOU (HLAVNÉ OKNO) ---
    def stlacenie_klavesy_hlavne(self, event):
        ...
        try:
            ...
            if index != -1 and index < self.pocet_riadkov:
                # TU BOLA CHYBA V STAREJ LOGIKE - TERAZ VOLÁME LEN OTVORENIE OKNA
                self.otvorit_vyber_chyby(index)
        except Exception: pass

    # --- OSTATNÉ FUNKCIE (Email, Varovanie, PIN, CSV, Menu) ---
