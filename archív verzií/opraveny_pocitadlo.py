...
# Poznámka: LIMIT_VAROVANIA je ponechané, ale logika používa LIMITY["warning"]
LIMIT_VAROVANIA = 20

# EMAIL (heslo odporúčané načítať z ENV - ak nie je nastavené, použije sa fallback)
ODOSIELATEL_EMAIL = "tvoj_email@gmail.com"
ODOSIELATEL_HESLO = os.environ.get("ODOSIELATEL_HESLO", "abcd efgh ijkl mnop") 
...
class PocitadloApp:
    def __init__(self, root):
        self.root = root
        self.root.title(f"Počítadlo Nepodarkov v{VERZIA}")
        ...
        try:
            ...
            logging.info("Fullscreen attribute nie je podporované, pokračujem v okne.")
        ...
        for i in range(3):
            ...
            if os.path.exists(nazov_suboru):
                try:
                    ...
                    ikony.append(img)
                    logging.info(f"Načítaný obrázok {nazov_suboru}")
                except Exception as e:
                    logging.exception(f"Nepodarilo sa načítať {nazov_suboru}, vytváram náhradný obrázok.")
                    ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
            else:
                logging.info(f"Obrázok {nazov_suboru} neexistuje, používam náhradný obrázok.")
                ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
        return ikony

    def vytvor_zastupny_obrazok(self, farba):
        img = tk.PhotoImage(width=100, height=100)
        img.put(farba, to=(0, 0, 99, 99))
        return img

    # --- POPUP OKNO S ONESKORENÍM ---
    def otvorit_vyber_chyby(self, index_riadku):
        logging.info(f"Otváram okno pre riadok {index_riadku} -> {NAZVY_TLACIDIEL[index_riadku]}")
        self.klavesnica_blokovana = True
        nazov_chyby = NAZVY_TLACIDIEL[index_riadku]
        
        popup = tk.Toplevel(self.root)
        popup.title(f"Detail chyby - {nazov_chyby}")
        ...
        # Dizajn okna
        frame_shadow = tk.Frame(popup, bg="#aaa")
        frame_shadow.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        def potvrdit_vyber(sub_id):
            logging.info(f"Potvrdený typ {sub_id} pre riadok {index_riadku}")
            popup.destroy()
            self.klavesnica_blokovana = False
            self.pridat_kus(index_riadku, sub_id)
        
        def zrusit_akciu():
            logging.info("Zrušené v detaile chyby")
            self.klavesnica_blokovana = False
            popup.destroy()
        ...
        # --- DÔLEŽITÁ OPRAVA: ONESKORENÉ VIAZANIE KLÁVES ---
        # Klávesnicu aktivujeme až po 300ms, aby sa nestalo "dvojité kliknutie"
        def nabinduj_klavesy_popup():
            popup.bind("1", lambda e: potvrdit_vyber(0))
            popup.bind("2", lambda e: potvrdit_vyber(1))
            popup.bind("3", lambda e: potvrdit_vyber(2))
            popup.bind("<Escape>", lambda e: zrusit_akciu())
            logging.debug("Klávesové viazania v popup okne aktivované.")

        popup.after(300, nabinduj_klavesy_popup)
        popup.focus_set()
    ...
    def pridat_kus(self, index_riadku, sub_id):
        ...
        # Používame LIMITY["warning"] aby boli prahy centralizované
        if nova_hodnota >= LIMITY.get("warning", LIMIT_VAROVANIA) and not self.varovanie_zobrazene[index_riadku]:
    ...
    def odobrat_kus(self, index_riadku):
        ...
        if self.sub_counters[index_riadku][last_sub_id] > 0:
    ...
    def prekresli_bar(self, index):
        try:
            ...
            logging.exception("Chyba pri získavaní canvas/rect/text pre index %s", index)
            return
        ...
        palette = COLORS_OK
        # Používame centralizované LIMITY
        if total_count > LIMITY.get("warning", 10): palette = COLORS_WARN
        if total_count > LIMITY.get("critical", 25): palette = COLORS_CRIT
        
        current_x = 0
        effective_total = max(total_count, 1)
        scale_factor = min(total_count / LIMITY.get("max_scale", 35), 1.0)
        total_bar_pixels = int(width * scale_factor)
        
        # Vypočítame segmenty a využijeme posledný segment na zvyšné pixely, aby bar vyplnil priestor
        for i in range(3):
            sub_count = counts[i]
            if sub_count > 0:
                if i == 2:
                    # posledný segment zoberie zvyšok pixlov
                    segment_width = max(0, total_bar_pixels - current_x)
                else:
    ...
    def stlacenie_klavesy_hlavne(self, event):
        # ak je klávesnica zablokovaná, ignoruj
        if self.klavesnica_blokovana:
            return

        znak = event.char or ""
        # Podpora aj pre numerickú klávesnicu (keysym)
        if not znak:
        ...
        try:
            index = -1
            if znak in [str(i) for i in range(1,10)]: index = int(znak) - 1
            elif znak == "0": index = 9
            elif znak == "+": index = 10
            ...
            logging.exception("Chyba pri spracovaní klávesy: %s", repr(znak))

    def poslat_email_vlakno(self, chyba, pocet, obsluha):
        if not POUZIT_EMAIL:
            logging.info("POUZIT_EMAIL je False — email sa neodosiela.")
            return
        try:
            msg = EmailMessage()
            ...
            s.send_message(msg)
            s.quit()
            logging.info("Email odoslaný pre %s (%s ks).", chyba, pocet)
        except Exception:
            logging.exception("Chyba pri odosielaní emailu.")

    def spustit_odosielanie_emailu(self, c, p):
        t = threading.Thread(target=self.poslat_email_vlakno, args=(c, p, self.aktualny_uzivatel_meno), daemon=True)
        t.start()
    ...
    def vyziadat_pin(self, nadpis, cb):
        ...
        v = tk.StringVar()
        tk.Label(w, textvariable=v, font=("Arial",24), width=10).pack(pady=10)
        ...
        # Podpora kláves - čísla, Enter, Escape
        def on_key(e):
    ...
    def overit_pin_pre_menu(self, pin):
        if pin in UZIVATELIA:
            self.otvorit_menu(UZIVATELIA[pin], pin)
        else:
            messagebox.showerror("Chyba", "Nesprávny PIN kód.")
    ...
    def skontroluj_csv(self):
        if not os.path.exists(SUBOR_HISTORIA):
            try:
                ...
                logging.exception("Chyba pri vytváraní CSV súboru histórie.")
    ...
    def zapis_do_historie(self, typ, chyba, val, user_name=None, user_pin=None):
        ...
        try:
            ...
            logging.exception("Chyba pri zapisovaní do histórie.")
    ...
    def manualny_export(self, user, pin):
        ...
        try:
            ...
            logging.exception("Chyba pri exporte súboru")
    ...
    def aktualizuj_cas(self):
        ...
        # kontrola resetu v sekundách (napr. "06:00:00")
        if t.strftime("%H:%M:%S") in [x + ":00" for x in CAS_RESETU]:
            logging.info("Spúšťam automatický reset (%s)", t.strftime("%H:%M:%S"))
    ...
    def aktualizuj_header_info(self):
        try:
            ...
            logging.exception("Chyba pri aktualizovaní header info.")

    def otvorit_menu(self, u, p):
        self.klavesnica_blokovana = True
        w = tk.Toplevel(self.root); w.geometry("600x650"); w.transient(self.root); w.grab_set(); w.configure(bg=COLOR_BG)
