...
# --- VERZIA ---
VERZIA = "22.0 (Detailed Defect Report)"
...
class PocitadloApp:
    ...
    # --- GENERÁTOR DETAILNÉHO REPORTU ---
    def generuj_report_text(self):
        """Vytvorí textový súhrn s rozpisom chýb."""
        text = ""
        total_sum = 0
        
        for i in range(self.pocet_riadkov):
            pocet_celkom = self.counters[i].get()
            
            if pocet_celkom > 0:
                nazov_modelu = NAZVY_TLACIDIEL[i]
                text += f"► {nazov_modelu}: CELKOM {pocet_celkom} ks\n"
                
                # Rozpis pod-typov
                counts = self.sub_counters[i] # [počet_nedopenena, počet_prestriekana, počet_roztrhnuta]
                
                for j in range(3):
                    if counts[j] > 0:
                        nazov_chyby = SUB_TYPES[j]['label']
                        text += f"    • {nazov_chyby}: {counts[j]} ks\n"
                
                text += "-" * 35 + "\n" # Oddeľovacia čiara
                total_sum += pocet_celkom
    ...
    def zobrazit_sumar_okno(self, nadpis, obsah_textu, je_to_reset=False):
        ...
        tk.Label(content_frame, text=f"Čas výpisu: {datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')}", 
