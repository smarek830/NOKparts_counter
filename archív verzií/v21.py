...
# --- VERZIA ---
VERZIA = "21.0 (Shift Report + Live Status)"

# --- FARBY MATERIAL DESIGN ---
COLOR_PRIMARY = "#263238"      # Hlavička (Tmavá)
...
COLOR_INFO_BTN = "#006064"     # Tmavá tyrkysová (Tlačidlo Sumár)
...
class PocitadloApp:
    ...
    def vytvor_zastupny_obrazok(self, farba):
        img = tk.PhotoImage(width=450, height=450)
        img.put(farba, to=(0, 0, 449, 449))
        return img

    # --- GENERÁTOR TEXTOVÉHO REPORTU ---
    def generuj_report_text(self):
        """Vytvorí textový súhrn aktuálneho stavu."""
        text = ""
        total_sum = 0
        for i in range(self.pocet_riadkov):
            pocet = self.counters[i].get()
            if pocet > 0:
                nazov = NAZVY_TLACIDIEL[i]
                text += f"{nazov}: {pocet} ks\n"
                total_sum += pocet
        
        if total_sum == 0:
            text = "Žiadne nepodarky."
        
        return text, total_sum

    # --- OKNO SO SUMÁROM (REPORT) ---
    def zobrazit_sumar_okno(self, nadpis, obsah_textu, je_to_reset=False):
        """Zobrazí okno so sumárom. Ak je to reset, upozorní na koniec smeny."""
        ...
        w.geometry(f"{width}x{height}+{int((sw-width)/2)}+{int((sh-height)/2)}")
        w.overrideredirect(True) # Bez rámu
        
        # Farba hlavičky podľa typu (Info vs Reset)
        hlavicka_bg = COLOR_INFO_BTN if not je_to_reset else COLOR_PRIMARY
        
        # Hlavička
        header = tk.Frame(w, bg=hlavicka_bg, height=80)
        header.pack(fill=tk.X)
        header.pack_propagate(False)
        tk.Label(header, text=nadpis, font=("Arial", 24, "bold"), fg="white", bg=hlavicka_bg).pack(pady=20)
        
        # Obsah
        content_frame = tk.Frame(w, bg="white", padx=20, pady=20)
        content_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Info o užívateľovi
        tk.Label(content_frame, text=f"Zodpovedná obsluha: {self.aktualny_uzivatel_meno}", 
                 font=("Arial", 16, "bold"), fg="#555", bg="white").pack(anchor="w", pady=(0, 10))
        
        tk.Label(content_frame, text=f"Čas výpisu: {datetime.datetime.now().strftime('%H:%M:%S')}", 
                 font=("Arial", 12), fg="#888", bg="white").pack(anchor="w", pady=(0, 20))
        
        # Scrollovateľný text (ak by bolo veľa chýb)
        text_area = tk.Text(content_frame, font=("Courier New", 14, "bold"), height=15, width=50, relief="flat", bg="#f5f5f5")
        text_area.insert(tk.END, obsah_textu)
        ...
        tk.Button(w, text="ZATVORIŤ OKNO", font=("Arial", 18, "bold"), bg="#ddd", command=zavriet).pack(side=tk.BOTTOM, fill=tk.X, pady=20, padx=50)
        
        w.wait_visibility()
        w.grab_set()

    # --- AKCIA PRE TLAČIDLO "AKTUÁLNY STAV" ---
    def ukaz_aktualny_sumar(self):
        report, total = self.generuj_report_text()
        full_text = f"CELKOM NEPODARKOV: {total}\n--------------------------\n{report}"
        self.zobrazit_sumar_okno("AKTUÁLNY STAV SMENY", full_text, je_to_reset=False)

    # --- POPUP VÝBER CHYBY (Bez zmeny) ---
    def otvorit_vyber_chyby(self, index_riadku):
        ...
        header = tk.Frame(content, bg=COLOR_PRIMARY, height=100)
        header.pack(fill=tk.X)
        header.pack_propagate(False)
        tk.Label(header, text=f"DETAIL CHYBY: {nazov_chyby}", font=("Arial", 32, "bold"), bg=COLOR_PRIMARY, fg="white").pack(pady=25)

        grid_container = tk.Frame(content, bg="white")
        grid_container.pack(fill=tk.BOTH, expand=True, padx=20, pady=(60, 5))
        ...
        for i in range(3):
            col_frame = tk.Frame(grid_container, bg="white")
            col_frame.grid(row=0, column=i, sticky="nsew", padx=10)
            
            cfg = SUB_TYPES[i]
            
            btn = tk.Button(col_frame, image=self.ikony_chyb[i], 
                            bg="#F5F5F5", relief="raised", bd=3,
                            command=lambda s=i: potvrdit_vyber(s))
            btn.pack(pady=(0, 15)) 

            lbl_name = tk.Label(col_frame, text=cfg['label'], font=("Arial", 22, "bold"), bg="white", fg="#333")
            lbl_name.pack(pady=5)

            lbl_key = tk.Label(col_frame, text=f"({i+1})", font=("Arial", 28, "bold"), bg="white", fg=COLOR_ACCENT)
            lbl_key.pack(pady=5)

        btn_cancel = tk.Button(content, text="ZRUŠIŤ (ESC)", font=("Arial", 18), bg="#ddd", command=zrusit_akciu)
        btn_cancel.pack(side=tk.BOTTOM, fill=tk.X, pady=15, padx=30)
    ...
    def stlacenie_klavesy_hlavne(self, event):
        ...
        if znak in ["1", "2", "3", "4", "5", "6", "7", "8", "9"]: index = int(znak) - 1
        elif znak == "0": index = 9
        elif znak == "+": index = 10
    ...
    def vytvor_header(self):
        hf = tk.Frame(self.root, bg=COLOR_PRIMARY, height=130); hf.pack(side=tk.TOP, fill=tk.X); hf.pack_propagate(False)
        ...
        self.lbl_cas = tk.Label(rf, text="00:00", font=("Roboto", 48, "bold"), fg=COLORS_OK[1], bg=COLOR_PRIMARY); self.lbl_cas.pack(anchor="e")
        self.lbl_datum = tk.Label(rf, text="...", font=("Roboto", 18), fg="#B0BEC5", bg=COLOR_PRIMARY); self.lbl_datum.pack(anchor="e")
        
        # MENO A TLAČIDLO SUMÁR
        self.lbl_uzivatel = tk.Label(hf, text="-", font=("Roboto", 28, "bold"), fg="#FFFF00", bg=COLOR_PRIMARY); self.lbl_uzivatel.pack(side=tk.RIGHT, padx=40)
        
        # NOVÉ TLAČIDLO SUMÁR V HLAVIČKE
        btn_sumar = tk.Button(hf, text="AKTUÁLNY STAV", font=("Arial", 12, "bold"), bg=COLOR_INFO_BTN, fg="white", 
                              relief="raised", command=self.ukaz_aktualny_sumar)
        btn_sumar.pack(side=tk.RIGHT, padx=10, ipady=5)

        sh = tk.Frame(self.root, bg=COLOR_SECONDARY, height=60); sh.pack(side=tk.TOP, fill=tk.X); sh.pack_propagate(False)
        ...
        for i, txt in enumerate(labels):
            tk.Label(sh, text=txt, font=("Arial", 16, "bold"), fg="white", bg=COLOR_SECONDARY).grid(row=0, column=i, sticky="ew", pady=15)
    ...
    def otvorit_menu(self, u, p):
        ...
        btns = [
            ...
            ("2. RESET", lambda: [self.potvrdit_reset(u,p, w), close()], COLORS_WARN[1]),
            ("3. REŠTART", lambda: self.potvrdit_restart(u,p, w), COLORS_CRIT[1]),
            ("4. ZMENA OBSLUHY", lambda: [close(), self.vyziadat_pin("ZMENA", self.spracuj_prihlasenie)], "#607D8B"),
            ("5. UKONČIŤ", lambda: self.potvrdit_ukoncenie(u,p, w), "black")
        ]
        for t, c, col in btns:
    ...
    # --- DIALÓGY S PARENT PARAMETROM ---
    def potvrdit_ukoncenie(self, m, p, parent_win):
        if messagebox.askyesno("Exit", "Vypnúť?", parent=parent_win): 
            self.zapis_do_historie("UKONČENIE", "SYS", 0, m, p)
            self.root.destroy()
    def potvrdit_reset(self, m, p, parent_win):
        if messagebox.askyesno("Reset", "Vymazať?", parent=parent_win): self.reset_vsetko(False, m, p)
    def potvrdit_restart(self, m, p, parent_win):
        if messagebox.askyesno("Reštart", "Reštartovať?", parent=parent_win): 
    ...
    def reset_vsetko(self, automaticky=False, user_name=None, user_pin=None):
        ...
        # --- 1. Generuj Report PRED vymazaním ---
        report_text, _ = self.generuj_report_text()
        full_report = f"REPORT UKONČENEJ SMENY\n----------------------\n{report_text}"
        ...
        # --- 2. Ak je to automatický reset, ukáž report ---
        if automaticky:
            self.zobrazit_sumar_okno("KONIEC SMENY", full_report, je_to_reset=True)
