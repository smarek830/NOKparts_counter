...
# --- VERZIA ---
VERZIA = "17.0 (Fix Rýchlych Kláves)"
...
class PocitadloApp:
    ...
    # --- KĽÚČOVÁ OPRAVA: POPUP OKNO S ONESKORENÍM ---
    def otvorit_vyber_chyby(self, index_riadku):
        # 1. Okamžite zablokujeme hlavnú klávesnicu, aby sa nedalo klikať ďalej
        ...
        def potvrdit_vyber(sub_id):
            popup.destroy()
            self.klavesnica_blokovana = False # Odblokujeme až po zatvorení
        ...
        # --- DÔLEŽITÉ: AKTIVÁCIA KLÁVESNICE AŽ PO 500ms ---
        # Týmto zabránime, aby stlačenie "1" na hlavnej obrazovke
        # omylom stlačilo aj "1" v tomto okne.
        def aktivovat_klavesy_popup():
            popup.bind("1", lambda e: potvrdit_vyber(0))
            popup.bind("2", lambda e: potvrdit_vyber(1))
            popup.bind("3", lambda e: potvrdit_vyber(2))
            
            # Podpora pre NUMPAD (Numerickú klávesnicu)
            ...
            # Niekedy numpad posiela End/Down ak nie je NumLock
            ...
            print("Popup klávesnica aktivovaná.")

        # Oneskorenie 500ms (pol sekundy)
        popup.after(500, aktivovat_klavesy_popup)
        popup.focus_set()
    ...
    def stlacenie_klavesy_hlavne(self, event):
        ...
        # Podpora pre horné čísla aj Numpad
        index = -1
        
        # Prevod znakov na indexy
