...
class PocitadloApp:
        ...
        for i in range(3):
            ...
            if os.path.exists(nazov_suboru):
                try:
                    ...
                    ikony.append(img)
                    print(f"OK: Načítaný obrázok {nazov_suboru}")
                except Exception as e:
                    print(f"CHYBA: Nedá sa načítať {nazov_suboru}: {e}")
                    ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
            else:
                print(f"INFO: Obrázok {nazov_suboru} neexistuje, používam farbu.")
                ikony.append(self.vytvor_zastupny_obrazok(COLORS_OK[i]))
        return ikony
    ...
    def otvorit_vyber_chyby(self, index_riadku):
        print(f"Otváram okno pre riadok: {index_riadku + 1}")
        ...
        def nabinduj_klavesy_popup():
            ...
            print("Klávesnica v okne aktivovaná.")
