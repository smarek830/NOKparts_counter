...
class PocitadloApp:
    ...
    def vytvor_zastupny_obrazok(self, farba):
        # Obrázok 300x300 pixelov
        img = tk.PhotoImage(width=600, height=600)
        img.put(farba, to=(0, 0, 599, 599))
        return img

    # --- POPUP OKNO (NOVÝ LAYOUT) ---
    def otvorit_vyber_chyby(self, index_riadku):
        ...
        # Hlavička
        header = tk.Frame(content, bg=COLOR_POP_HEAD, height=100)
        header.pack(fill=tk.X)
        header.pack_propagate(False)
        tk.Label(header, text=f"DETAIL CHYBY: {nazov_chyby}", font=("Arial", 32, "bold"), bg=COLOR_POP_HEAD, fg="white").pack(pady=25)

        # Kontajner pre stĺpce
        grid_container = tk.Frame(content, bg="white")
        grid_container.pack(fill=tk.BOTH, expand=True, padx=5, pady=(60, 5))
        grid_container.columnconfigure(0, weigh=1)
        ...
        # --- GENERUJEME 3 STĹPCE ---
        for i in range(3):
            # Kontajner pre jeden stĺpec (Column Frame)
            ...
            # height/width v Button pri obrázkoch nefunguje ako pixely, ale veľkosť sa prispôsobí obrázku
            img = self.ikony_chyb[i]
            btn = tk.Button(col_frame, image=img, 
                            bg="#F5F5F5", relief="raised", bd=3,
                            command=lambda s=i: potvrdit_vyber(s))
            # DÔLEŽITÉ: Uložíme referenciu na obrázok priamo na tlačidlo, aby ho garbage collector neodstránil
            btn.image = img 
            btn.pack(pady=(0, 35)) # Medzera pod obrázkom

            # 2. NÁZOV CHYBY (Pod obrázkom)
            lbl_name = tk.Label(col_frame, text=cfg['label'], font=("Arial", 22, "bold"), bg="white", fg="#333")
            lbl_name.pack(pady=5)

            # 3. ČÍSLO KLÁVESY (V krúžku - simulované)
    ...
    def stlacenie_klavesy_hlavne(self, event):
        ...
        key_map = {
            ...
            'KP_1': 0, 'KP_2': 1, 'KP_3': 2, 'KP_4': 3, 'KP_5': 4, 'KP_6': 5, 'KP_7': 6, 'KP_8': 7, 'KP_9': 8,
            'KP_0': 9, 'KP_Add': 10
        }

        index = -1
        if event.char in key_map:
            index = key_map[event.char]
        elif event.keysym in key_map:
            index = key_map[event.keysym]
    ...
    # --- OPRAVA SCHOVANÝCH DIALÓGOV (Pridaný parameter parent) ---
